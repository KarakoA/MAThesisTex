...
% 3 coloured buttons? %
%a button that resets all
% one that mixes colours

% or list with 3 elem and buttons do things
...
As can be seen most clearly in \ref{eval:reactions_meal}, The 'New Day' button changes the 'Meal of the day' and all 'Price' list HTML tags. Only the price of each meal is changed, the 'meals[i].name' HTML tags remain the same. 'Best steak in town' is also not updated, since steak is never the daily meal. 

Clicking on 'Discount all' does indeed change both 'Best steak in town' and all 'Price' HTML tags, since everything is discounted.
