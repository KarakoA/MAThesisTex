...
\ref{fig:diagram_list_complex} shows an extension to the Math for Kids application. The full source code can be found it \ref{appendix:math_kids_extended_source_code}
...
, used to display the capabilities of the interaction diagram and Gherkin scenario template generation application when handling lists and computed properties.

%TODO if-else comment on. way to diferentiate currently, since if-else in code not suported ytet
