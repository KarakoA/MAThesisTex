...
The scenario generation is implemented in the \code{scenario.ts} file and currently ouputs the obtained scenarios, tags, nodes react to and the gherkin scenarios templates to the console. The scenarios are generated exactly as described in \ref{concept:scenario_generation}
...
A web server can be startet in order to view the results in a browser at \code{localhost:8000} via
%lang bash
\begin{lstlisting}
npm run serve
\end{lstlisting}
